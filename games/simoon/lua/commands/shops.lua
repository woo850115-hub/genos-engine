-- shops.lua — Simoon shop commands (buy, sell, list, appraise)
    ...
    for i = 0, 100 do
        ...
        if name_lower:find(target_name, 1, true) then
            ...
            if ctx.char.gold < price then
                ctx:send("골드가 부족합니다. (" .. price .. "골드 필요)")
                return
            end
            ctx.char.gold = ctx.char.gold - price
            local obj = ctx:buy_item(item)
            if obj then
                ctx:send("{green}" .. obj.name .. "을(를) " .. price .. "골드에 구입했습니다.{reset}")
            end
            return
        end
    end
...
register_command("sell", function(ctx, args)
    ...
    ctx:sell_item(item)
    ctx.char.gold = ctx.char.gold + price
    ctx:send("{green}" .. item.name .. "을(를) " .. price .. "골드에 팔았습니다.{reset}")
end, "팔")
...
register_command("list", function(ctx, args)
    ...
    for i = 0, 100 do
        ...
        if not ok or not item then break end
        local price = ctx:get_buy_price(item)
        lines[#lines + 1] = string.format("  %3d) %-20s %8d골드",
            i + 1, item.name, price)
    end
...
register_command("appraise", function(ctx, args)
    ...
    local price = ctx:get_sell_price(item)
    ctx:send(item.name .. "의 판매가: " .. price .. "골드")
end, "감정")
